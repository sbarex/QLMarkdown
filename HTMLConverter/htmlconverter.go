...
func convertCodeToHTML(source *C.char, lexer *C.char, theme *C.char) *C.char {
	sourceString := convertToGoString(source)
	lexerString := convertToGoString(lexer)
	themeString := convertToGoString(theme)
	...
	if l == nil {
		// errMessage := fmt.Sprintf("error: missing lexer")
...
func getCSSStyle(lexer *C.char, theme *C.char, background *C.char) *C.char {
	lexerString := convertToGoString(lexer)
	themeString := convertToGoString(theme)
	...
	style := styles.Get(themeString)
	styleBuffer := new(bytes.Buffer)

	err := formatter.WriteCSS(styleBuffer, style)
	if err != nil {
		errMessage := fmt.Sprintf("<style type='text/css'>/** error: Could not render css style: %d */</style>\n", err)
		return convertToCString(errMessage)
	}
	styleString := styleBuffer.String()
	styleString = strings.Replace(styleString, ".chroma", "pre.chroma", -1)

	backgroundString := convertToGoString(background)
	// fmt.Println("background: ", backgroundString)
	if backgroundString == "ignore" {
		// Remove the theme background.
		var re = regexp.MustCompile(`/\* Background \*/\s+pre\.chroma\s+\{\s+background-color:\s+#[a-f0.9]{6}\s*}`)
		styleString = re.ReplaceAllString(styleString, "")
	} else if backgroundString != "" {

		color := chroma.ParseColour(backgroundString)
		// Remove the theme background.
		var re = regexp.MustCompile(`(/\* Background \*/\s+pre\.chroma\s+\{\s+background-color:\s+)(#[a-f0.9]{6})(\s*})`)
		styleString = re.ReplaceAllString(styleString, `$1`+color.String()+`$3`)
	}
	return convertToCString("<style type='text/css'>pre.chroma { color: #000000; }" + styleString + "</style>\n")
}

//export getStyles
// getStyles return the names of the available styles.
func getStyles() *C.char {
	t, _ := json.Marshal(styles.Names())
	return convertToCString(string(t))
}

//export addStyle
func addStyle(name *C.char, settings *C.char) int {
	lo := getLogger()
	defer lo.Release()

	settingsString := convertToGoString(settings)
	var result map[string]string
	json.Unmarshal([]byte(settingsString), &result)

	builder := chroma.NewStyleBuilder(convertToGoString(name))
	...
	for key, element := range result {
		...
		switch key {
		case "Background":
			token = chroma.Background
		case "LineNumbers":
			token = chroma.LineNumbers
		case "LineNumbersTable":
			token = chroma.LineNumbersTable
		case "LineHighlight":
			token = chroma.LineHighlight
		case "LineTable":
			token = chroma.LineTable
		case "LineTableTD":
			token = chroma.LineTableTD
		case "Error":
			token = chroma.Error
		case "Other":
			token = chroma.Other
		case "None":
			token = chroma.None
		case "EOFType":
			token = chroma.EOFType

		// Keywords.
		case "Keyword":
			token = chroma.Keyword
		case "KeywordConstant":
			token = chroma.KeywordConstant
		case "KeywordDeclaration":
			token = chroma.KeywordDeclaration
		case "KeywordNamespace":
			token = chroma.KeywordNamespace
		case "KeywordPseudo":
			token = chroma.KeywordPseudo
		case "KeywordReserved":
			token = chroma.KeywordReserved
		case "KeywordType":
			token = chroma.KeywordType

		// Names.
		case "Name":
			token = chroma.Name
		case "NameAttribute":
			token = chroma.NameAttribute
		case "NameBuiltin":
			token = chroma.NameBuiltin
		case "NameBuiltinPseudo":
			token = chroma.NameBuiltinPseudo
		case "NameClass":
			token = chroma.NameClass
		case "NameConstant":
			token = chroma.NameConstant
		case "NameDecorator":
			token = chroma.NameDecorator
		case "NameEntity":
			token = chroma.NameEntity
		case "NameException":
			token = chroma.NameException
		case "NameFunction":
			token = chroma.NameFunction
		case "NameFunctionMagic":
			token = chroma.NameFunctionMagic
		case "NameKeyword":
			token = chroma.NameKeyword
		case "NameLabel":
			token = chroma.NameLabel
		case "NameNamespace":
			token = chroma.NameNamespace
		case "NameOperator":
			token = chroma.NameOperator
		case "NameOther":
			token = chroma.NameOther
		case "NamePseudo":
			token = chroma.NamePseudo
		case "NameProperty":
			token = chroma.NameProperty
		case "NameTag":
			token = chroma.NameTag
		case "NameVariable":
			token = chroma.NameVariable
		case "NameVariableAnonymous":
		...
		case "NameVariableMagic":
			token = chroma.NameVariableMagic

		// Literals.
		case "Literal":
			token = chroma.Literal
		case "LiteralDate", "Date":
			token = chroma.LiteralDate
		case "LiteralOther":
			token = chroma.LiteralOther

		// Strings.
		case "LiteralString", "String":
			token = chroma.LiteralString
		case "LiteralStringAffix", "StringAffix":
			token = chroma.LiteralStringAffix
		case "LiteralStringAtom":
			token = chroma.LiteralStringAtom
		case "LiteralStringBacktick", "StringBacktick":
			token = chroma.LiteralStringBacktick
		case "LiteralStringBoolean":
			token = chroma.LiteralStringBoolean
		case "LiteralStringChar", "StringChar":
			token = chroma.LiteralStringChar
		case "LiteralStringDelimiter", "StringDelimiter":
			token = chroma.LiteralStringDelimiter
		case "LiteralStringDoc", "StringDoc":
			token = chroma.LiteralStringDoc
		case "LiteralStringDouble", "StringDouble":
			token = chroma.LiteralStringDouble
		case "LiteralStringEscape", "StringEscape":
			token = chroma.LiteralStringEscape
		case "LiteralStringHeredoc", "StringHeredoc":
			token = chroma.LiteralStringHeredoc
		case "LiteralStringInterpol", "StringInterpol":
			token = chroma.LiteralStringInterpol
		case "LiteralStringName":
			token = chroma.LiteralStringName
		case "LiteralStringOther", "StringOther":
			token = chroma.LiteralStringOther
		case "LiteralStringRegex", "StringRegex":
			token = chroma.LiteralStringRegex
		case "LiteralStringSingle", "StringSingle":
			token = chroma.LiteralStringSingle
		case "LiteralStringSymbol", "StringSymbol":
			token = chroma.LiteralStringSymbol

		// Numbers.
		case "LiteralNumber", "Number":
			token = chroma.LiteralNumber
		case "LiteralNumberBin", "NumberBin":
			token = chroma.LiteralNumberBin
		case "LiteralNumberFloat", "NumberFloat":
			token = chroma.LiteralNumberFloat
		case "LiteralNumberHex", "NumberHex":
			token = chroma.LiteralNumberHex
		case "LiteralNumberInteger", "NumberInteger":
			token = chroma.LiteralNumberInteger
		case "LiteralNumberIntegerLong", "NumberIntegerLong":
			token = chroma.LiteralNumberIntegerLong
		case "LiteralNumberOct", "NumberOct":
			token = chroma.LiteralNumberOct

		// Operators.
		case "Operator":
			token = chroma.Operator
		case "OperatorWord":
			token = chroma.OperatorWord

		// Punctuation.
		case "Punctuation":
			token = chroma.Punctuation

		// Comments.
		case "Comment":
			token = chroma.Comment
		case "CommentHashbang":
			token = chroma.CommentHashbang
		case "CommentMultiline":
			token = chroma.CommentMultiline
		case "CommentSingle":
			token = chroma.CommentSingle
		case "CommentSpecial":
			token = chroma.CommentSpecial

		// Preprocessor "comments".
		case "CommentPreproc":
			token = chroma.CommentPreproc
		case "CommentPreprocFile":
			token = chroma.CommentPreprocFile

		// Generic tokens.
		case "Generic":
			token = chroma.Generic
		case "GenericDeleted":
			token = chroma.GenericDeleted
		case "GenericEmph":
			token = chroma.GenericEmph
		case "GenericError":
			token = chroma.GenericError
		case "GenericHeading":
			token = chroma.GenericHeading
		case "GenericInserted":
			token = chroma.GenericInserted
		case "GenericOutput":
			token = chroma.GenericOutput
		case "GenericPrompt":
			token = chroma.GenericPrompt
		case "GenericStrong":
			token = chroma.GenericStrong
		case "GenericSubheading":
			token = chroma.GenericSubheading
		case "GenericTraceback":
			token = chroma.GenericTraceback
		case "GenericUnderline":
			token = chroma.GenericUnderline

		// Text.
		case "Text":
			token = chroma.Text
		case "TextWhitespace", "Whitespace":
			token = chroma.TextWhitespace
		case "TextSymbol":
			token = chroma.TextSymbol
		case "TextPunctuation":
			token = chroma.TextPunctuation
		default:
			lo.Errorf("Unknown key: %s => %s", key, element)
			// fmt.Println("Unknown key:", key, "=>", element)
			continue
		}

		style, err := chroma.ParseStyleEntry(element)
		if err == nil {
			// fmt.Println(token, ": ", style)
			builder.AddEntry(token, style)
		}
	}
...
func containsEmoji(text *C.char) bool {
	textString := convertToGoString(text)

	emoji := getEmoji()
	for key, _ := range emoji {
		if strings.Contains(textString, ":"+key+":") {
			// fmt.Println("found :", key, ": on ", textString)
			return true
		}
	}
	return false
...
//export replaceEmoji
func replaceEmoji(text *C.char, use_characters bool) *C.char {
	textString := convertToGoString(text)

	emoji := getEmoji()

	for key, address := range emoji {
		if !strings.Contains(textString, ":"+key+":") {
			continue
		}
		var replace string
		if use_characters {
			...
			//replace = ""
			rune := []int32{}
			strconv.ParseInt("10", 16, 32)
			for _, character := range characters {
				b, _ := strconv.ParseInt(character, 16, 32)
				rune = append(rune, int32(b))
				//replace += "&#x" + character + ";"
			}
			replace = B2S(rune)
		} else {
			replace = "<img src='" + address + "' alt='" + key + "' width='16' height='16' />"
		}
		textString = strings.Replace(textString, ":"+key+":", replace, -1)
	}
	return convertToCString(textString)
}

//export getStyleInfo
func getStyleInfo(name *C.char) *C.char {
	nameString := convertToGoString(name)
	style := styles.Get(nameString)
	...
	for i, type_ := range types {
		info[type_.String()] = style.Get(type_).String()
		fmt.Println(i, type_, style.Get(type_))
	}
...
func base64encoding(filename *C.char, basedir *C.char) *C.char {
	file := convertToGoString(filename)

	lo := getLogger()
	defer lo.Release()

	lo.Infof("Request base64 encoding of `%s`", file)
	u, err := url.Parse(file)
	if err != nil {
		return nil
	}
	if u.Scheme != "" && u.Scheme != "file" {
		// fmt.Println("Ignore scheme: " + u.Scheme)
		lo.Infof("The scheme `%s` is not supported.", u.Scheme)
		return nil
	}
	...
	if err != nil {
		// fmt.Println(err)
		lo.Errorf("Unable to detect the mime for file `%s`.", file)
		return nil
	}
	if !strings.HasPrefix(mime.String(), "image/") {
		// fmt.Printf("Mime %s is not allowed\n", mime)
		lo.Errorf("The mime `%s` is not an image.", mime.String())
		return nil
	}
	...
	f, err := os.Open(file)
	if err != nil {
		lo.Errorf("Unable to read file `%s` : %s.", file, err)
		// fmt.Println(err)
		return nil
	}
	defer f.Close()
	...
	encoded = "data:" + mime.String() + ";base64," + encoded
	return convertToCString(encoded)
}
